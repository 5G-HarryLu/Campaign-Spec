...
依 [規格 Ch05 — Data Model](./index.html#p5) 產出的 10 張 jackpot 系列建表語法,供手動建立。
...
- 所有 `DATETIME` 一律 **UTC**（應用層負責，不用 DB 時區函式）
...
> ⚠ OceanBase 相容 MySQL 語法，以下 4 張 OceanBase 表沿用 MySQL DDL 風格；若要指定 OB 分區 / replica / locality 需另依環境補上（規格未定義,依 DBA 慣例）。
...
CREATE TABLE `jackpot_round` (
  ...
  `opened_at`                              DATETIME         NOT NULL COMMENT 'UTC',
  `closed_at`                              DATETIME         NULL     COMMENT 'UTC',
  PRIMARY KEY (`id`),
...
CREATE TABLE `jackpot_win` (
  ...
  `created_at`                  DATETIME         NOT NULL COMMENT 'UTC',
  `credited_at`                 DATETIME         NULL     COMMENT 'UTC',
  `notified_at`                 DATETIME         NULL     COMMENT 'UTC',
  `updated_at`                  DATETIME         NOT NULL COMMENT 'UTC',
  PRIMARY KEY (`id`),
...
CREATE TABLE `jackpot_pool_snapshot` (
  ...
  `snapshot_at`     DATETIME         NOT NULL COMMENT '自然鍵複合主鍵 3(本身即冪等,免 ObjectId)',
  `round_id`        VARCHAR(24)      NOT NULL COMMENT '所屬回合',
...
- **`status` / `enabled` 預設值**：規格未逐欄指定 DEFAULT,以上 `DEFAULT` 為合理推斷（pool/config status、operator enabled、payout_status=1 pending、round status=1 open、retry_count=0）。若與團隊慣例不符請調整。
- **`NOT NULL` 判定**：規格標「NULL」者（round 的 `clamp_overflow` / `closed_at`、win 的 `failure_reason` / `credited_at` / `notified_at`、config 的 `roll_range_json`、audit 的 `pool_group_id` / `tier` / `before_json` / `after_json`）保留可空,其餘一律 NOT NULL。
